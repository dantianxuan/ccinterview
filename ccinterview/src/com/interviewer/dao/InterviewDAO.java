 ...
 * Interview entities. Transaction control of the save(), update() and delete()
 * operations can directly support Spring container-managed transactions or they
 * can be augmented to handle user-managed Spring transactions. Each of these
 * methods provides additional information for how to configure it for the
 * desired type of transaction control.
 * 
 * @see com.interviewer.dao.Interview
 * @author MyEclipse Persistence Tools
 */

public class InterviewDAO extends BaseHibernateDAO {
	private static final Logger log = LoggerFactory
			.getLogger(InterviewDAO.class);
	// property constants
	...
	public Interview findById(java.lang.Integer id) {
		...
		try {
			Interview instance = (Interview) getSession().get(
					"com.interviewer.dao.Interview", id);
			return instance;
		} catch (RuntimeException re) {
	...
	public List findByExample(Interview instance) {
		...
		try {
			List results = getSession()
					.createCriteria("com.interviewer.dao.Interview")
	...
	public List findByProperty(String propertyName, Object value) {
		log.debug("finding Interview instance with property: " + propertyName
				+ ", value: " + value);
		try {
			String queryString = "from Interview as model where model."
					+ propertyName + "= ?";
	...
	public List findByInterviewerId(Object interviewerId) {
		return findByProperty(INTERVIEWER_ID, interviewerId);
	}
