 ...
 * RegMail entities. Transaction control of the save(), update() and delete()
 * operations can directly support Spring container-managed transactions or they
 * can be augmented to handle user-managed Spring transactions. Each of these
 * methods provides additional information for how to configure it for the
 * desired type of transaction control.
 * 
 * @see com.interviewer.dao.RegMail
...
public class RegMailDAO extends BaseHibernateDAO {
	...
	public RegMail findById(com.interviewer.dao.RegMailId id) {
		log.debug("getting RegMail instance with id: " + id);
		try {
			RegMail instance = (RegMail) getSession().get(
					"com.interviewer.dao.RegMail", id);
			return instance;
		} catch (RuntimeException re) {
	...
	public List findByExample(RegMail instance) {
		...
		try {
			List results = getSession()
					.createCriteria("com.interviewer.dao.RegMail")
	...
	public List findByProperty(String propertyName, Object value) {
		log.debug("finding RegMail instance with property: " + propertyName
				+ ", value: " + value);
		try {
			String queryString = "from RegMail as model where model."
					+ propertyName + "= ?";
